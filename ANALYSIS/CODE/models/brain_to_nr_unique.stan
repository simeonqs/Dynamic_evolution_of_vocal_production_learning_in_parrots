...
// Description: Model to estimate the effect of brain size. 
...
generated quantities{
  ...
  brain_merge = merge_missing(brain_missidx, to_vector(stand_brain), brain_impute);
  for(n in 1:N_species) pred_brain[n] = a_brain + b_body_brain * stand_body[n];
  for(i in 1:N_obs) rel_brain[i] = brain_merge[species[i]] - pred_brain[species[i]];
}
