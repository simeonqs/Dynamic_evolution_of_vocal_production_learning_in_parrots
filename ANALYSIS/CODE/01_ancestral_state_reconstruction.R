...
path_pdf_tree_words = 'ANALYSIS/RESULTS/tree human words.pdf'
...
# Rerun ASR for human words ----
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

# Reload data, to get complete tree
load(path_cleaned_data)

# Load long data to find human words
load(path_cleaned_data_long)

# Add human words
dat$human_words = vapply(dat$species, function(sp){
  if(!sp %in% dat_long$species) y = 0 else {
    y = ifelse(sum(dat_long$distinctwords[dat_long$species == sp]) > 0,
               1, 0)
  }
}, numeric(1))

# Run ASR
ph = data.frame(value = dat$human_words)
rownames(ph) = tree$tip.label
svl = as.matrix(ph)[,1]
obj = contMap(tree, svl, outline = FALSE, plot = F)
